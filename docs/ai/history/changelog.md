...
## [HIRA-017] Daily Lesson Tab (10 Vocab + 3 Kanji) — 2026-04-16

Thêm learning mode độc lập `Học mỗi ngày` trên home, gồm overview 25 ngày, learn flow theo block vocab/kanji, mixed quick practice, summary, và lưu trạng thái `not_started / in_progress / completed` qua `progress.js`. Daily Lesson dùng schema mở rộng với `topic`, `category`, `note`, `audioText`, `pronunciationVi`, `example`; đã map dữ liệu thật cho `day01-day04`, còn `day05+` giữ placeholder theo schema mới để fill tiếp sau. Vocab learn card được enrich với nút loa, dòng phát âm gần đúng, 1 câu ví dụ, 1 dòng dịch nghĩa, và highlight từ đang học.

Files: `src/index.html`, `src/style.css`, `src/progress.js`, `src/dailyLessonsData.js`, `src/dailyLesson.js`

---

## [HIRA-016] Migrate Repo Workflow from Claude Style to Codex Style — 2026-04-12

Thiết lập workflow Codex-first cho repo: tạo `AGENTS.md` làm entrypoint mới ở root, thêm `.codex/config.toml`, map workflow procedural sang `docs/ai/playbooks/`, rút `CLAUDE.md` xuống thành compatibility shim, và cập nhật `scripts/verify.sh` sang wording AI-workflow-neutral. Giữ `.claude/rules/*`, `.claude/skills/*`, và các legacy settings như lớp transitional để migration an toàn và dễ review.

Files: `AGENTS.md`, `.codex/config.toml`, `CLAUDE.md`, `scripts/verify.sh`, `.claude/rules/*.md`, `.claude/skills/*/SKILL.md`, `docs/ai/playbooks/*.md`
